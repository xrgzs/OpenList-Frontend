...
/**
 * 执行周期编辑器：以表格呈现预设式配置（每月/每周/每天/每N小时/每N分钟/自定义）多条执行时间。
 * 自带单行预览（cron + 未来 5 次执行时间）和「近5次执行」Tooltip，
 * 通过 onChange 以 cron 表达式数组的形式向父组件同步结果。
 */
export const CronSpecsEditor = (props: CronSpecsEditorProps) => {
  const t = useT()

  /** 执行周期行；每条都是一个预设（每月/每周/每天/每N小时/每N分钟/自定义）。 */
  ...
  return (
    <>
      {/* 执行周期：预设式配置，支持每月/每周/每天/每N小时/每N分钟/自定义多条。 */}
      <FormControl
        w="$full"
        display="flex"
